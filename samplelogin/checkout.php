...
if (isset($_POST["submit"])) {
    ...
    // Perform validation and sanitization on user input
    ...
    while ($row = mysqli_fetch_array($result)) {
        ...
        $id = $row['id'];
        $qty = $row['qty'];
        $sql = "SELECT invoice_no FROM payment WHERE Cid='$cid' AND status='Not Paid' AND Amount=''";
        ...
        $sql = "UPDATE payment SET Amount='$total' WHERE invoice_no='$invno'";
    ...
    if ($pmod == "upi") {
        ?>
            <form method="post" action="upipay.php?no=<?php echo $invno; ?>">
            <div class="form-group">
                Enter UPI id: <input type="text" name="upid" placeholder="UPI id" required>
            </div><br>
                <button type="submit" name="cont"><?php echo "Pay Rs. ".$total; ?></button>
            </form>
            <?php
    } 
}
